...
# script to download the dataset from the html file

tables = \
    pd.read_html('https://www.ref.org.uk/generators/index.php?start=0&order=AvAnnMWh&dir=desc'
                 )
generators = tables[1]

for i in range(100, 15500, 100):  # the dataset was in 155 pages, with 100 rows each
    table = \
        pd.read_html('https://www.ref.org.uk/generators/index.php?start=%s&order=AvAnnMWh&dir=desc'
                      % i)
    generators_sub_list = table[1]
    generators = generators.append(generators_sub_list,
                                   ignore_index=True)

generators.to_csv  # save the table to CSV format
...
fp = \
    '/Countries_December_2017_Full_Extent_Boundaries_in_UK.shp'
